...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale; id: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: cleanPath, languages: { "es-ES": `/es/photo/${photo.id}`, "en-US": `/en/photo/${photo.id}` } },
...
export default async function PhotoPage({ params }: { params: Promise<{ locale: Locale; id: string }> }) {
  ...
  return <PageShell>
    ...
    <PhotoDetailPage locale={locale} photoId={photo.id} imagesData={imagesData} dictionary={dictionary} />
