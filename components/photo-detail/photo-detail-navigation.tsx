...
export function PhotoDetailNavigation({ dictionary, galleryDictionary }: { dictionary: PhotoDetailDictionary; galleryDictionary: GalleryDictionary }) {
  ...
  const item = (photo: typeof previous, direction: "previous" | "next") => {
    ...
    return <Link rel={direction === "previous" ? "prev" : "next"} href={hrefFor(photo.id)} onClick={() => preserveContext(photo.id)} className="photo-detail-nav-card group" data-press-feedback {...getCursorTargetAttributes({ type: direction, contrast: "dark" })}>
