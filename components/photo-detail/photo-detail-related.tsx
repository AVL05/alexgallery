...
export function PhotoDetailRelated({ dictionary, galleryDictionary }: { dictionary: PhotoDetailDictionary; galleryDictionary: GalleryDictionary }) {
  ...
  return <section aria-labelledby="related-title">
    ...
    <div className="mt-8 grid gap-x-5 gap-y-10 sm:grid-cols-2 lg:grid-cols-4">{related.map((photo) => <article key={photo.id}><Link href={hrefFor(photo.id)} onClick={() => preserveContext(photo.id)} className="group block" data-press-feedback {...getCursorTargetAttributes({ type: "view", contrast: "dark" })}><div className="relative overflow-hidden bg-[var(--color-surface)]" style={{ aspectRatio: `${photo.width}/${photo.height}` }}><Image src={photo.src} alt={photo.alt || photo.description} fill sizes="(max-width: 639px) 100vw, (max-width: 1023px) 50vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-[1.015] group-focus-visible:scale-[1.015]" /></div><h3 className="mt-4 font-serif text-xl leading-tight transition-colors group-hover:text-accent group-focus-visible:text-accent">{photo.title}</h3><p className="rv-meta mt-2">{galleryDictionary.categories[photo.category]} / {photo.year}</p></Link></article>)}</div>
  </section>;
}
